...
class Staff(Person):
    '''
    This subclass Staff is-a Person. This subclass will change the tax rate
    ...
    def __init__(self):
        ...
        self._tax = 0.09
    '''
    We override the method from Person since we know this class is a Staff,
    therefore the tax variable is already set to 0.09
    '''
    def saveToFile(self):
        '''
        ...
        time stamps; small change - output file will state "Staff Bill"
        '''
        Flag1 = True
        while Flag1:
            if self._total == 0:
                ...
                Directory = os.getcwd()
                if path.isdir("Staff Receipts"):
                     print("Receipt Added to Staff Receipt Folder")
                else:
                     os.chdir(Directory)
                     os.mkdir("Staff Receipts")
                Directory = os.path.join(Directory,"Staff Receipts")
                timeStamp = time.time()
                ...
                with open(orderTimeStamp,"w") as fileHandleToSaveTheBill:
                    fileHandleToSaveTheBill.write("Staff Bill:\n")
